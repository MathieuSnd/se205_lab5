...
future_t * submit_callable (executor_t * executor, callable_t * callable) {
  ...
  pthread_mutex_init(&future->mut, NULL);
  pthread_cond_init(&future->complete_cond, NULL);
...
void * get_callable_result (future_t * future) {
  ...
  pthread_mutex_lock(&future->mut);

  while(!future->completed)
    pthread_cond_wait(&future->complete_cond, &future->mut);

  result = (void *) future->result;
  
  // Unprotect against concurrent accesses
  pthread_mutex_unlock(&future->mut);
...
void * main_pool_thread (void * arg) {
  ...
  while (future != NULL) {
    ...
    pthread_mutex_lock(&future->mut);
    future->completed = 1;
    pthread_cond_broadcast(&future->complete_cond);
    pthread_mutex_unlock(&future->mut);
...
void executor_shutdown (executor_t * executor) {
  ...
  for(unsigned i = 0; i < 4; i++) {
    protected_buffer_put(executor->futures, NULL);
  }
